...
1. Сучасні інструменти збирання ([webpack](https://webpack.js.org/) та інші) об’єднують модулі разом, оптимізують їх для пришвидшення завантаження та видаляють невикористані частини.
...
Модулі використовують спеціальний синтаксис `export default` ("експорт за уставою") для створення єдиної сутності та полегшення доступності.
...
| Іменований експорт | Експорт за уставою |
|--------------|----------------|
| `export class User {...}` | `export default class User {...}` |
| `import {User} from ...` | `import User from ...`|

Технічно, ми можемо використовувати іменовані та експорти за уставою в одному модулі, але, на практиці, їх не заведенл змішувати. Модулі повинні мати або іменований експорт, або експорт за уставою.

Оскільки, тільки одна сутність може бути типово експортованою, вона може не мати імені.

В прикладі правильно використано експорти за уставою:
...
В деяких ситуаціях ключове слово `default` використовують як ім'я для позначення експорту за уставою.
...
Чи, в іншій ситуації, скажімо модуль `user.js` експортує єдину сутність "за уставою" та ще декілька іменованих (рідко, але таке трапляється):
...
Для імпорту сутності "за уставою" та декількох іменованих потрібно:
